...
def process_permno_chunk(task: tuple) -> tuple[dict, dict]:
    (permno_chunk, ohlc_path, features_path, calendar, window_days_list,
     as_of_per_window, worker_id, tmp_root) = task
    ...
    for i, permno in enumerate(permno_chunk):
        ...
        feature_panel = read_stock_features(features_path, int(permno))
        for window_days in window_days_list:
            for as_of in as_of_per_window[window_days]:
                ...
                labels[(window_days, as_of.year)].append(
                    labels_for_as_of(feature_panel, as_of, int(permno), window_days)
                )
