...
#   ./scripts/sh/06_holding_breakdown.sh --market cn
#   ./scripts/sh/06_holding_breakdown.sh --market us --direct-signal
#
set -euo pipefail
...
PY="$ROOT/scripts/py/06_holding_breakdown.py"
if [[ ! -f "$PY" ]]; then
...
if [[ -z "${TMUX:-}" ]]; then
  ...
  mkdir -p "$ROOT/logs/06_holding_breakdown"
  STAMP="$(date +%Y%m%d_%H%M%S)"
  SESSION="rpt_06_bd_${STAMP}"
  MAIN_LOG="$ROOT/logs/06_holding_breakdown/06_holding_breakdown_${STAMP}.log"
  ...
  tmux new-session -d -s "$SESSION" \
    ...
    bash -lc "cd '$ROOT' && bash '$ROOT/scripts/sh/06_holding_breakdown.sh'${_quoted_args}"
...
LOG_DIR="$ROOT/logs/06_holding_breakdown"
mkdir -p "$LOG_DIR"
STAMP="${RPT_STAMP:-$(date +%Y%m%d_%H%M%S)}"
MAIN_LOG="${RPT_MAIN_LOG:-$LOG_DIR/06_holding_breakdown_${STAMP}.log}"
...
if [[ "$ec" -ne 0 ]]; then
  log "ERROR: 06_holding_breakdown failed (exit ${ec}). See ${MAIN_LOG}"
  exit "$ec"
fi
